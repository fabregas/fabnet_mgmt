...
def create_ca_ks(output_file, password, role_name, root_ca_ks=None, CN=None):
    ...
    role = ROLES_MAP.get(role_name, None)
    if root_ca_ks and not role:
    ...
    if not root_ca_ks:
        ...
        ca_private, ca_cert = generate_ca(2048, None, role, 100*365, CN)
        print ' -> OK'
    else:
        ...
        print ' -> generating key and certificate for %s role (role=%s, serial=%s)'%(role_name, role, count)
        ...
        cl_ca_req = gen_request(ca_private, CN, None, \
                S.CERT_C, S.CERT_L, S.CERT_O, role, None, S.CERT_MAIL)
